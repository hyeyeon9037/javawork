...
public class ServerProgram {
   /*
    현재 구동중인 서버가 있으면 클라이언트는 그 서버로 요청을 보낼 수 있음 => 응답결과가 돌아옴
    - 요청과 응답에 의해서 프로그램이 돌아감("통신"한다라고 표현)
    - 웹에서는 통신방식 : http 프로토콜 통신(https)
    
    자바만을 가지고 서버와 클라이언트 간에 간단한 통신하기
    이 때 데이터를 입출력하고자 한다면 서버와 클라이언트간에 스트림(연결통로)가 필요
    
    소켓 : 프로세스간의 통신을 담당. 스트림을 연결하기 위한 문.
    소켓 프로그래밍
    - TCP : 데이터를 정확하고 안정적으로 전달 가능(신뢰성). 그대신 느림 (이메일, http)
    - UDP : 신뢰성이 없는 데이터 전달. 그대신 빠름(실시간 동영상 서비스)
    */ 
	
   /*
    TCP(Treansmission Control Protocol)
    - 서버, 클라이언트 1:1 소켓 통신
    - 데이터를 교환하기 전에 서버, 클라이언트가 연결되어 있어야함
    
    Socket
    - Input/OutputStream을 가지고 소켓통신을 함
    
    ServerSocket
    - 포트와 여결(Bind)되어 외부에 연결요청을 기다리다 연결 요청이 들어오면 수락해줄 용도
   ...
   public static void main(String[] args) {
      ...
      try {
         ...
         Socket socket = server.accept(); //통신할 수 있는 소켓 생성
         ...
         br = new BufferedReader(new InputStreamReader(socket.getInputStream())); //위에껄 한줄로
         pw = new PrintWriter(socket.getOutputStream());
         //데이터 출력시 print(), println 메소드를 가지고 있는 보조 스트림
         
         pw.println("환영합니다 (´▽`ʃ♡ƪ)");
